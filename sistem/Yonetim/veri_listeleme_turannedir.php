...
$islem = $_GET["islem"];
    ...
    if ($islem== "ekle") {
        ...
        $metin2 = $_POST["metin2"];
        $id = $_POST["id"];
        $sorgu = $baglan->query("INSERT INTO turannedir (baslik1, baslik2, metin1, metin2)
         VALUES ('$baslik1', '$baslik2', '$metin1', '$metin2')");      
        echo "<script> window.location.href='veri_listeleme_turannedir.php'; </script>";
    }
...
--<a href="veri_listeleme_kerkuk.php">KERKÜK ÖNEMİ</a>-<a href="cikis.php" onclick="if(!confirm('çıkış yapmak istediğinize emin misiniz'))return false;">ÇIKIŞ</a>
        ...
        <form action="veri_listeleme_turannedir.php?islem=ekle" enctype="multipart/form-data" method="post">
     <b>baslik1:</b><input type="text" size="20" name="baslik1"> 
     <b>baslik2:</b><input type="text" size="20" name="baslik2"> 
    ...
    <div style="margin-bottom:20px;  padding: 20px; border: 2px solid red;">
        <table>
            ...
            while($satir = $sonuc->fetch_assoc())
            {
                echo '<tr>';
                echo '<td>'.$satir['id'].'</td>'; 
                echo '<td>'.$satir['baslik1'].'</td>';
                echo '<td>'.$satir['baslik2'].'</td>';
                echo '<td>'.$satir['metin1'].'</td>';
                echo '<td>'.$satir['metin2'].'</td>';
                echo '<td>';
                echo '<a href="duzenle_turannedir.php?id='.$satir['id'].'" onclick="closeCurrentTab()">Düzenle</a>&nbsp;&nbsp; ';    
                echo ' <a href="sil_turannedir.php?id='.$satir['id'].'" onclick="closeCurrentTab()">Sil</a>';
                echo '</td>';
                echo '</tr>';
            }
            ?>
